    ...
    _createClass(OwnsMany, [{
        ...
        value: function on(action, callback) {
            ...
            Object.keys(this._models).forEach(function (id) {
                _this3._listeners[action].push({
                    model: _this3._models[id],
                    event: _this3._models[id].on(action, callback)
                });
            });
        }
        ...
        value: function requestMany() {
            ...
            model.getAdapter().findAll(this.Model, this._parent, options, function (response) {
                return _this4.apply(response).applyQueryResults(response, options);
            });
        ...
        value: function apply(data) {
            ...
            data.forEach(function (item) {
                var model = _this5._instantiateModel();

                _this5._models[item.id] = model;
                model.apply(item);
            });
